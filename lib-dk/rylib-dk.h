// Ry Library Development Kit (RyLib-DK)
#ifndef RY_LIB_DK_H
#define RY_LIB_DK_H
...
// The function signature for native functions loaded from a dynamic library.
typedef RyValue (*RyNativeFn)(int argCount, RyValue *args, std::map<std::string, RyValue> &globals);
typedef void (*RyRegisterFn)(const char *name, RyNativeFn fn, int arity, void *target);

extern "C" {
void init_ry_module(RyRegisterFn reg, void *target);
}
